...
class EducationModuleTestCase(APITestCase):
    ...
    def test_edu_module_patch(self):
        ...
        response = self.client.patch(
            f"/education/modules/{self.edu_module.id}/", data=data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class EducationModuleModelTest(TestCase):
    ...
    def test_verbose_names(self):
        ...
        self.assertEqual(
            EducationModule._meta.verbose_name_plural, "Образовательные модули"
        )
...
class LessonModelTest(TestCase):
    ...
    def test_model(self):
        ...
        self.assertEqual(
            str(f"{self.edu_module.id} - {self.lesson.title}"),
            f"{self.edu_module.id} - test_lesson",
        )
...
class EducationModuleSerializerTestCase(TestCase):
    ...
    def test_edu_module_serializer(self):
        modul_1 = EducationModule.objects.create(
            title="test 1", description="test_description_1"
        )
        modul_2 = EducationModule.objects.create(
            title="test 2", description="test_description_2"
        )
        data = EducationModuleSerializer([modul_1, modul_2], many=True).data
        expected_data = [
            {"id": modul_1.id, "title": "test 1", "description": "test_description_1"},
            {"id": modul_2.id, "title": "test 2", "description": "test_description_2"},
        ]
